...
impl GpuSearcher {
    pub fn new(tail_cells: &[u8], quadgrams: &[f32]) -> Result<Self, Box<dyn std::error::Error>> {
        if tail_cells.len() != 563 {
            return Err(format!(
                "GPU contract requires 563 tail cells, got {}",
                tail_cells.len()
            )
            .into());
        }
        ...
        let free_symbols: Vec<u8> = FREE_SYMBOLS
            ...
            .collect();
        let fixed = FixedData {
            tail_cells: stream0.clone_htod(tail_cells)?,
