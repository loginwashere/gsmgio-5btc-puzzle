...
use cpu::{
    decode_rank, decoded_cells_for_square, score_mean, score_tail, sha256_hex, square_for_rank,
    validate_contract, QuadgramModel, BASE_SQUARE, EXPECTED_DECODED_SHA256, FACTORIAL_16,
    TARGET_PREFIX,
};
...
#[derive(Parser, Debug)]
#[command(
    name = "bifid_gpu_search16",
    version,
    about = "GPU search of the sealed-BTCSEED 16! Bifid alphabet family"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
enum Command {
    ...
    /// Run or resume a rank interval. Full 16! launch requires explicit invocation.
    Search {
        #[arg(long, default_value_t = 0)]
        start: u64,
        #[arg(long, default_value_t = FACTORIAL_16)]
        end_exclusive: u64,
        #[arg(long, default_value_t = 2_048)]
        grid_size: u32,
        #[arg(long, default_value_t = 64)]
        ...
        retain_block_winners: usize,
        #[arg(long, default_value = "checkpoints/bifid_16factorial.json")]
        checkpoint: PathBuf,
        #[arg(long, default_value = "output/bifid_16factorial_result.json")]
        output: PathBuf,
        #[arg(long, default_value_t = false)]
        skip_self_test: bool,
    },
...
struct RunReport {
    ...
    projected_full_16factorial_seconds: Option<f64>,
    projected_full_16factorial_hours: Option<f64>,
...
fn fingerprint(model: &QuadgramModel, cells: &[u8], range_start: u64, end: u64) -> Fingerprint {
    Fingerprint {
        version: 1,
        family: format!(
            "bifid_16factorial_prefix_dependency_fixed_dynamic_coordinates_lexicographic;range_start={range_start}"
        ),
...
fn cpu_self_test() {
    ...
    let cells = decoded_cells_for_square(BASE_SQUARE);
    assert_eq!(cells[TARGET_PREFIX.len()..].len(), 563);
    for rank in [
        0,
        1,
        2,
        ...
        let score = score_tail(rank, &model);
...
fn gpu_self_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let gpu = gpu::GpuSearcher::new(&faed, &model.logs)?;
    ...
    for rank in [
        ...
        let expected = score_tail(rank, &model);
    ...
    let mut cpu_best = Winner {
        rank: 0,
        score_total: score_tail(0, &model),
    };
    for rank in 1..10_000 {
        let candidate = Winner {
            rank,
            score_total: score_tail(rank, &model),
        };
...
fn execute_gpu_range(
    ...
    if start >= end || end > FACTORIAL_16 {
        return Err(
            format!("invalid range [{start}, {end}); family ends at {FACTORIAL_16}").into(),
        );
    }
    let model = QuadgramModel::load_embedded();
    let cells = decoded_cells_for_square(BASE_SQUARE);
    ...
    let completed = gpu.scan(
        ...
        |batch_start, batch_count, rows| {
            ...
            if batch_number % 100 == 0 || next_rank == end {
    ...
    retained.sort_by(compare_winners);
    let detailed: Vec<_> = retained.iter().map(|winner| detail(winner, 563)).collect();
    let report = RunReport {
        tool: "bifid_gpu_search16",
        mode: mode.into(),
        device,
        range_start: resume,
        ...
        projected_full_16factorial_seconds: (rate > 0.0).then_some(FACTORIAL_16 as f64 / rate),
        projected_full_16factorial_hours: (rate > 0.0)
            .then_some(FACTORIAL_16 as f64 / rate / 3600.0),
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Command::CpuScore { rank } => {
            if rank >= FACTORIAL_16 {
                return Err(format!("rank must be below {FACTORIAL_16}").into());
            }
            cpu_self_test();
            let model = QuadgramModel::load_embedded();
            let score = score_tail(rank, &model);
