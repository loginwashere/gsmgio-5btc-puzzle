...
template <typename T>
static bool read_exact(T *target, size_t count) {
  return std::fread(target, sizeof(T), count, stdin) == count;
}

int main() {
  char magic[8];
  if (!read_exact(magic, size_t(8)) || std::memcmp(magic, "P484NG1\0", 8)) return 3;
  unsigned char blocks[WIDTH * ROWS];
  double coef[FEATURES], intercept;
  if (!read_exact(blocks, size_t(WIDTH * ROWS)) ||
      !read_exact(coef, size_t(FEATURES)) || !read_exact(&intercept, size_t(1))) return 3;
  ...
  if (d_scores) CUDA_OK(cudaFree(d_scores));
  if (d_paths) CUDA_OK(cudaFree(d_paths));
  CUDA_OK(cudaFree(d_coef)); CUDA_OK(cudaFree(d_blocks));
  return 0;
}
