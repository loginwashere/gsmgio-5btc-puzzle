#!/usr/bin/env python3
"""Phase 423: eligibility correction for the Phase 422 macro-clue panel.
...
EVIDENCE_PACKET_PATH = SCRIPT_DIR / "phase423_sealed_evidence_packet.txt"
PROMPT_PATH = SCRIPT_DIR / "phase423_frozen_solver_prompt.txt"
LAUNCHER_PATH = SCRIPT_DIR / "phase423_frozen_launcher.txt"
...
PROMPT_LENGTH = 16551
PROMPT_SHA256 = "b4e58b85a5d840fdd76d6bf0943ce36be079e284dc4e470ef91039f0c8411726"
PROMPT_LINE_COUNT = 257
LAUNCHER_LENGTH = 672
LAUNCHER_SHA256 = "d99030279ba641886449a9f37f8ba90ec632516de0522cf281d384297bd61ae4"

PERMITTED_COMMAND = (
    "sed -n '1,320p' "
    "/home/loginwashere/projects/gsmgio-5btc-puzzle/tools/gsmg/"
    "phase423_frozen_solver_prompt.txt"
)
...
# Complete case-insensitive intersection of Phase 416 forbidden vocabulary and
# the exact solver-visible prompt. All other residue checks remain live.
DISCLOSED_RESPONSE_RESIDUES = (
    "SALPH", "SALPHASEION", "SalPhaseIon", "yinyang",
)

def disclosed_response_residues(text):
    """Derive allowed residue classes from exact solver-visible bytes."""
    lowered = text.lower()
    return tuple(term for term in phase420.phase416.FORBIDDEN_VOCAB
                 if term.lower() in lowered)

def response_blinding_violations(text):
    """Retain the inherited filter except for frozen prompt disclosures."""
    allowed = {term.lower() for term in DISCLOSED_RESPONSE_RESIDUES}
    return [violation for violation in phase420.phase416.blinding_violations(text)
            if violation.lower() not in allowed]


def validate_invocation_record(record):
    """Apply Phase 420's record contract with Phase 423's exact command."""
    ...
    if response_blinding_violations(phase420.phase416.submission_full_text(submission)):
...
def build_prompt():
    ...
    if disclosed_response_residues(text) != DISCLOSED_RESPONSE_RESIDUES:
        raise AssertionError("solver-visible residue allowlist drifted")
...
def invocation_task_name(number):
    ...
    return f"phase423_invocation_{number}"
...
def self_test():
    ...
    assert invocation_task_name(1) == "phase423_invocation_1"
    assert invocation_task_name(8) == "phase423_invocation_8"
    ...
    for disclosed in DISCLOSED_RESPONSE_RESIDUES:
        disclosed_record = phase420._record(disclosed, telemetry=False)
        assert validate_invocation_record(disclosed_record)[0] is True
    for hidden in ("DBBI", "FAED", "Cosmic Duality", "phase 421", "github.com"):
        hidden_record = phase420._record(hidden, telemetry=False)
        assert validate_invocation_record(hidden_record)[0] is False
    mixed_record = phase420._record("SalPhaseIon then DBBI", telemetry=False)
    assert validate_invocation_record(mixed_record)[0] is False
