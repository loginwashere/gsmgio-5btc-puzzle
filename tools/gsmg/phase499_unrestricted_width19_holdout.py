#!/usr/bin/env python3
"""Checkpointed three-fixture holdout gate for the unrestricted width-19 solver."""
...
PROTOCOL = (REPO_ROOT / "doc" / "Brainstorms" /
            "2026-09-13 - Phase 499 Unrestricted Width19 Holdout Gate.md")
LOCK_PATH = SCRIPT_DIR / "phase499_execution_lock.json"
DEFAULT_WORK_ROOT = REPO_ROOT / "_work" / "phase499"
FIXTURE_INDICES = (0, 1, 2)
SPLIT = "holdout"
SWAP_COUNT = 3
...
def verify_lock() -> dict:
    if not LOCK_PATH.is_file():
        raise RuntimeError("Phase-499 execution lock is absent")
    actual = json.loads(LOCK_PATH.read_text())
    expected = expected_lock_payload()
    if actual != expected:
        raise RuntimeError("Phase-499 execution lock mismatch")
    return actual
...
def ensure_marker(work_dir: Path, fixture_index: int) -> None:
    ...
    if marker.exists():
        if json.loads(marker.read_text()) != expected:
            raise RuntimeError("Phase-499 marker mismatch")
    else:
...
def validated_front(work_dir: Path, fixture_index: int) -> Path | None:
    ...
    if not record_path.exists() or not checkpoint.exists():
        raise RuntimeError("partial Phase-499 front checkpoint")
    ...
    for key, value in expected.items():
        if record.get(key) != value:
            raise RuntimeError(f"Phase-499 front has wrong {key}")
    return checkpoint
...
def run_fixture(fixture_index: int, work_root: Path = DEFAULT_WORK_ROOT) -> dict:
    ...
    output = work_dir / "phase499_complete_result.json"
    if output.exists():
    ...
    def supply(requested_index=0, split="dev"):
        if requested_index == fixture_index and split == SPLIT:
            return fixture
        raise RuntimeError("unexpected fixture request in Phase 499")
...
def aggregate(work_root: Path = DEFAULT_WORK_ROOT) -> dict:
    ...
    for fixture_index in FIXTURE_INDICES:
        work_dir = Path(work_root) / f"i{fixture_index}_s{SWAP_COUNT}"
        path = work_dir / "phase499_complete_result.json"
        if not path.is_file():
    ...
    atomic_json(Path(work_root) / "phase499_holdout_result.json", result)
    return result
...
def self_test() -> dict:
    ...
    if "data" in imports:
        raise AssertionError("Phase 499 imports puzzle data")
