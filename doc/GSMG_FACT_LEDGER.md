...
| F-CHAIN-002 | Reading `574061` as a forward 2×3 digit matrix and taking row sums gives `[23,16,7]`; this exact list is independently reproduced by the `SALPHATION -> SALVATION` letter-count route and by the real *Matrix Reloaded* screenplay's dialogue counts (23 individuals/16 female/7 male) | authenticated-artifact | stable | positive | structural-only | 32, 58, 97, 250 | [GSMG_MATRIXSUMLIST_CHECKPOINT](GSMG_MATRIXSUMLIST_CHECKPOINT.md) |
...
| F-CHAIN-013 | The Phase 3.2 Architect monologue's own residual vocabulary ("source codes", "prime basics", "23 ciphers/16 encryptions/7 intertwined passwords", "actual private keynote", "brute forcing", "take the private key youve earned it") and the raw 149-digit `VALIDATION_NUM` decimal stream have zero hits as standard passphrases (19 candidates, 105 key materials, full CBC/stream/ECB/key-wrap oracle) against all 4 tracked blobs | authenticated-artifact | closed | negative | rejected | 265 | `tools/gsmg/phase32_monologue_residual_audit.py` |
| F-CHAIN-014 | Phase 3's exact seven-part concatenation (`causality/Safenet/Luna/HSM/11110/hex/chessFEN`, SHA-256 `1a57c572...`) and its digest, reused as a candidate P32TRAILING password, have zero hits (119 key materials, full standard oracle) against all 4 tracked blobs | authenticated-artifact | closed | negative | rejected | 266 | `tools/gsmg/phase3_sevenpart_p32_reuse_audit.py` |
| F-CHAIN-015 | Every sentence (mechanically split) of the decrypted Phase 2, Phase 3, and Phase 3.2 (including the Beaufort-decoded Architect speech, split on its own line breaks since it has no punctuation) text -- 48 candidates, 2,196 key materials -- has zero hits as a standard passphrase against all 4 tracked blobs | authenticated-artifact | closed | negative | rejected | 267 | `tools/gsmg/phase3_chain_full_text_p32_sweep_audit.py` |
| F-CHAIN-016 | X2SH4Y0QB15's literal text, numerically-substituted text (S=32/H=-42/B=-16/Q=82), coordinate-serialized text, and three "worst gear"=reverse scopes (whole-token, per-row, coordinate-swap) plus character reversal -- 24 candidates, 588 key materials -- have zero hits as a standard passphrase against all 4 tracked blobs | authenticated-artifact | closed | negative | rejected | 268 | `tools/gsmg/x2sh4y0qb15_p32_candidate_audit.py` |
| F-CHAIN-017 | Phase 269 corrects the bounded Phase 268 scope: the complete authenticated six-line X2SH4Y0QB15 block, literal/solved headers joined to the "worst gear" instruction, reversed coordinate-route serializations, and the original family (all forward/character-reversed) produce 42 pinned candidates / 1,362 non-empty key materials and zero hits against all 4 blobs; the oracle includes the previously omitted 20 Blowfish/Camellia/SEED CBC menu-gap variants | authenticated-artifact | closed | negative | rejected | 269 | `tools/gsmg/x2sh4y0qb15_p32_candidate_audit.py` |
| F-CHAIN-018 | The live-fetched, PKCS7-padding-verified Phase 2 plaintext (648 bytes, `SHA256(causality)` password) and Phase 3 plaintext (4,090 bytes, seven-part-concatenation password) contain zero `0x08` backspace bytes and zero other non-CRLF control bytes anywhere; Phase 2's plaintext matches README's transcription exactly and this exact ciphertext is corroborated stable since a 2020-03-25 urlscan.io capture | authenticated-artifact | closed | negative | rejected | 351 | `tools/gsmg/phase2_phase3_control_byte_audit.py` |
| F-CHAIN-019 | Rotating the authenticated 24 colored apertures by 90 degrees and retaining the established spiral/inverse reading yields a second six-digit prime, `311027`, beside `574061`. Inside the frozen matrix/Architect/frame/parity/affix family, exactly one eligible orientation reaches the authenticated prefix `THEFLOWER`; however, the operation family was formalized after seeing the target, Phase 464's preregistered audit of 5,901 creator messages and 88 media records found no exact reference or qualifying selector, direct blob consumers are negative, the same bounded grammar cannot continue to `BLOSSOMS`, and Phases 465-466 reject the selected old credential as an inherited modular running key or exact DBBI/FAED crib. Phase 467 retains the branch but leaves its operation, direction/representation, and consumer unbound | solver-derived | parked | partial | recognition-only | 461, 464-467 | [GSMG_ROSE_PRIME_CLOSED_CYCLE_AUDIT](GSMG_ROSE_PRIME_CLOSED_CYCLE_AUDIT.md); [GSMG_PHASE464_ROTATED_PRIME_PROVENANCE_AUDIT](GSMG_PHASE464_ROTATED_PRIME_PROVENANCE_AUDIT.md); [GSMG_PHASE465_PHASE1_RUNNING_KEY_AUDIT](GSMG_PHASE465_PHASE1_RUNNING_KEY_AUDIT.md); [GSMG_PHASE466_PHASE1_CREDENTIAL_CRIB_AUDIT](GSMG_PHASE466_PHASE1_CREDENTIAL_CRIB_AUDIT.md); [GSMG_PHASE467_CLOSED_SYSTEM_CONSTRAINT_CLOSURE](GSMG_PHASE467_CLOSED_SYSTEM_CONSTRAINT_CLOSURE.md) |
...
| F-OBJ-003 | FAED's independently-best `{g,i}` pair and the Architect-mirror-predicted `{h,e}` pair remain unreconciled. Neither pair's mirror9 image (`{a,c}` / `{c,g}`) validly segments its own origin stream — `{c,g}` cannot even segment FAED (dangling final escape). Page-authored markup supplies no third selector either: DBBI and FAED share one `<textarea>` text node with no DOM boundary, identical attributes, no distinguishing CSS/JS/comments, and uniform whitespace at the join. The SalPhaseIon textarea (DBBI/`matrixsumlist`/FAED and everything else in it) is confirmed byte-identical across 16 successful archive capture events (5 Wayback + 11 urlscan), 2023-05-31 to 2026-04-05; all 11 urlscan response hashes exactly match one of the first 3 authenticated Wayback HTML variants. The entire page-boundary branch is exhausted; the gap now depends entirely on a genuinely external selector. Phases 271-321 separately closed ~50 direct cryptanalytic/structural models over DBBI/FAED (independent of the page-boundary branch) without reconciling the two pairs | authenticated-artifact | stable | negative | structural-only | 225, 236, 243, 244, 249, 321 | [GSMG_MACRO_MODEL_DISPOSITION_AUDIT](GSMG_MACRO_MODEL_DISPOSITION_AUDIT.md); [GSMG_DBBI_FAED_BOUNDARY_SELECTOR_AUDIT](GSMG_DBBI_FAED_BOUNDARY_SELECTOR_AUDIT.md); [GSMG_SALPHASEION_URLSCAN_HISTORY_AUDIT](GSMG_SALPHASEION_URLSCAN_HISTORY_AUDIT.md) |
| F-OBJ-004 | DBBI's 91 base-9 symbols carry `91 log2(9) = 288.463...` bits of direct information: enough for a raw 256-bit scalar, but not for a conventional uncompressed (296-bit) or compressed (304-bit) WIF binary payload. The exclusion is only for direct whole-stream WIF representation; it does not reject raw keys or prior transforms | authenticated-artifact | stable | negative | structural-only | 330 | [GSMG_EXTERNAL_ARCHIVE_AUDIT](GSMG_EXTERNAL_ARCHIVE_AUDIT.md) |
| F-OBJ-005 | Across direct bijections of FAED's nine symbols to digits 0--8, the minimum whole-stream base-9 integer bit length is 1,801 in both forward and reverse order; therefore FAED-as-one-integer cannot equal exactly seven concatenated 32-byte keys (1,792 bits). Segmentation, record structures, and prior transforms remain outside this exclusion | authenticated-artifact | stable | negative | structural-only | 330 | [GSMG_EXTERNAL_ARCHIVE_AUDIT](GSMG_EXTERNAL_ARCHIVE_AUDIT.md) |
...
| F-CREATOR-001 | The creator Telegram identity (`Jrk Bgrt`/`@SoWut`, `user9815232`, same across both exports) repeatedly self-identifies as Netherlands-based in first-person statements, 2018–2022 ("I'm in the Netherlands even"; "Currently back in the NL"), plus a persistent Dutch-English "ofcourse" spelling tell. Team ("we"/"us") framing in one message means this establishes the account's Netherlands basis, not necessarily the individual designer's personal nationality | creator-primary | stable | positive | provenance-only | 245 | [GSMG_CREATOR_PERSONAL_DISCLOSURES_AUDIT](GSMG_CREATOR_PERSONAL_DISCLOSURES_AUDIT.md) |
| F-CREATOR-002 | The same creator identity made 7 casual references to recreational/psychedelic substance use across the solver-group export, 2021–2026, 4 of them clustered in one 2026-07-17 late-night session; none are framed as clues, hints, or puzzle-design commentary | creator-primary | stable | positive | provenance-only | 245 | [GSMG_CREATOR_PERSONAL_DISCLOSURES_AUDIT](GSMG_CREATOR_PERSONAL_DISCLOSURES_AUDIT.md) |
