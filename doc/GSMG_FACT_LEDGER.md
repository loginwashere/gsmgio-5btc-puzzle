...
| F-OBJ-003 | FAED's independently-best `{g,i}` pair and the Architect-mirror-predicted `{h,e}` pair remain unreconciled. Neither pair's mirror9 image (`{a,c}` / `{c,g}`) validly segments its own origin stream — `{c,g}` cannot even segment FAED (dangling final escape). Page-authored markup supplies no third selector either: DBBI and FAED share one `<textarea>` text node with no DOM boundary, identical attributes, no distinguishing CSS/JS/comments, and uniform whitespace at the join. The SalPhaseIon textarea (DBBI/`matrixsumlist`/FAED and everything else in it) is further confirmed byte-identical across all 5 known Wayback captures, 2023-06-01 to 2026-04-05 — the entire page-boundary branch is exhausted; the gap now depends entirely on a genuinely external selector | authenticated-artifact | stable | negative | structural-only | 225, 236, 243, 244 | [GSMG_MACRO_MODEL_DISPOSITION_AUDIT](GSMG_MACRO_MODEL_DISPOSITION_AUDIT.md); [GSMG_DBBI_FAED_BOUNDARY_SELECTOR_AUDIT](GSMG_DBBI_FAED_BOUNDARY_SELECTOR_AUDIT.md) |
