...
| 361 | QR residual compactness and center identifiability | All 11 held-out errors lie on one subpixel row and two columns (`p=1/501`), supporting a mechanical correction. Equally scoring center interactions give different continuations, proving the hidden center is not identifiable. |
| 360 | Six-variant QR `#FAFAFA` atlas | Row-plus-column model predicts 773/784 held-out pixels (`p=3/501`) and fits all observed bits, replacing the one-universal-tile interpretation with a position-aware six-variant model. |
