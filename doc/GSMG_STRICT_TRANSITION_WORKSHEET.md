...
| DBBI/FAED -> creator's `yinyang` state | **PASS**: creator describes yin-yang as reached state | **PASS**: authenticated DBBI and FAED streams | **FAIL**: no relationship/operator selected; Phase 454 recipient-side acquisition recovered no interaction-defining creator source or new page variant | **FAIL**: state is recognizable only semantically | **PASS** for ~50 further negative coupling families (Phases 271-321: transition matrices, mirror9, co-occurrence, GF(9)/base-27/base-81/FSM/sequence-alignment, Bacon, Nihilist, Cosmic-Duality running key, Bellaso, ADFGVX, Gronsfeld — see [GSMG_DBBI_FAED_BOUNDARY_SELECTOR_AUDIT](GSMG_DBBI_FAED_BOUNDARY_SELECTOR_AUDIT.md)), in addition to the families already covered as of 2026-08-09 | **Live semantic boundary, not executable** | Creator evidence defining how the streams interact |
...
| Matrix product -> `(255,103)` / `FF67` | **PARTIAL**: operands are real; multiplication is not named | **PASS**: `[[5,7,4],[0,6,1]]` and `[23,16,7]` | **FAIL**: reusing the sum list as vector and byte serialization are unselected | **PARTIAL**: `FF`/`g` thematic only | **PASS**: permutation/orientation family; Phase 453 FF-plus-letter endpoint unusual under both frozen nulls but unselected; Phase 456 source replay exact but 0/3 solved boundaries have a comparable native matrix/multiplication/byte-consumer grammar | **Strong arithmetic checkpoint; parked** | Clue selecting multiplication and a byte consumer |
| Second matrix list difference -> reversed `KIT` | **PARTIAL**: both source lists reproduce; subtraction/reversal not sourced | **PASS**: `[43,25,18]` and `[23,16,7]` | **FAIL**: subtraction, A1Z26, and reversal are three unselected steps | **PARTIAL**: young-rabbit word is thematic | **PASS**: one of eight bounded orientations; Phase 453 broader-word calibration null-sensitive | **Parked; never a password** | Clue explicitly asking for difference/reversal or a young rabbit |
| FEFE tuple `{1,4,21}` -> `ggn` -> secp256k1 | **PARTIAL**: hierarchical tuple is real; flattening is not sourced | **PASS** for tuple; **PARTIAL** for peer URL indices | **FAIL**: indexing convention, `g→G`, scalar `k`, negation, and curve are unselected | **PARTIAL**: exact `ggn`, but generic group narrative | **PASS**: 2,024 triples calibrated; Phase 453 structure common after Holm correction; curve narrative uncalibrated | **Narrative hypothesis; parked** | Independent clue supplies `k` and selects group order/negation |
